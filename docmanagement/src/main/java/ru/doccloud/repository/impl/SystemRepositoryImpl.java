...
import static ru.doccloud.repository.util.DataQueryHelper.jsonObject;
 ...
 */
@Repository
public class SystemRepositoryImpl extends AbstractJooqRepository implements SystemRepository<SystemDocument> {
    ...
    @Transactional
    @Override
    public SystemDocument add(String type, SystemDocument documentEntry) {
        ...
        SystemRecord persisted = jooq.insertInto(
                ...
                SYSTEM.SYS_FILE_NAME, SYSTEM.SYS_FILE_PATH, SYSTEM.SYS_VERSION, SYSTEM.SYS_SYMBOLIC_NAME, SYSTEM.SYS_PARENT_UUID)
                .values(
    ...
    @Transactional
    @Override
    public SystemDocument delete(String type, UUID id) {
        LOGGER.trace("entering delete(id={})", id);

        SystemDocument deleted = findById(type, id);
        ...
        int deletedRecordCount = jooq.delete(SYSTEM)
                .where(SYSTEM.SYS_UUID.equal(id))
                .execute();
    ...
    public Page<SystemDocument> findAll(Pageable pageable, String query) {
        ...
        List<SystemRecord> queryResults = jooq.selectFrom(SYSTEM)
                .where(cond)
                .orderBy(getSortFields(pageable.getSort(), SYSTEM, SYSTEM.DATA, null))
    ...
    @Override
    public Page<SystemDocument> findAllByType(String type, String[] fields, Pageable pageable, String query, JsonNode typeData) {
        ...
        selectedFields.add(SYSTEM.SYS_PARENT_UUID);
        if (fields!=null){
        	if (fields[0].equals("all")){
                selectedFields.add(SYSTEM.DATA);
            }else{
        ...
        List<Record> queryResults = jooq.select(selectedFields).from(SYSTEM)
                .where(cond)
                .orderBy(getSortFields(pageable.getSort(), SYSTEM, SYSTEM.DATA, propTypes))
                .limit(pageable.getPageSize()).offset(pageable.getOffset())
    ...
    @Override
    public Page<SystemDocument> findAllByParentAndType(UUID parentid, String type, Pageable pageable) {
        ...
        Condition cond = SYSTEM.SYS_TYPE.equal(type);
        cond = cond.and(SYSTEM.SYS_PARENT_UUID.equal(parentid));
        List<SystemRecord>  queryResults = jooq.selectFrom(SYSTEM)
                .where(cond)
                .orderBy(getSortFields(pageable.getSort(), SYSTEM, SYSTEM.DATA, null))
                .limit(pageable.getPageSize()).offset(pageable.getOffset())
    ...
    @Transactional(readOnly = true)
    @Override
    public SystemDocument findById(String type, UUID id) {
    ...
    @Transactional
    @Override
    public SystemDocument update(String type, SystemDocument documentEntry) {
        ...
        int updatedRecordCount = jooq.update(SYSTEM)
                ...
                .set(SYSTEM.SYS_PARENT_UUID, documentEntry.getParent())
                ...
                .where(SYSTEM.SYS_UUID.equal(documentEntry.getId()))
                .execute();
    ...
    @Transactional
    @Override
    public SystemDocument updateFileInfo(String type, SystemDocument documentEntry) {
