...
//    private static final Logger LOGGER = LoggerFactory.getLogger(FileBridgeTypeManager.class);
    ...
    private static final String ABSTARCT_SYSTEM = "abstract_system";
    private static final String DOCUMENT = "document";

    private final TypeDefinitionFactory typeDefinitionFactory;
    private final Map<String, TypeDefinition> typeDefinitions;
    private final SystemCrudService<SystemDTO> systemService;
    private final MutableFolderTypeDefinition folderType;
    private final MutableDocumentTypeDefinition documentType;

    public FileBridgeTypeManager(SystemCrudService<SystemDTO> sysService) throws Exception {
        ...
        folderType = typeDefinitionFactory
        ...
        documentType = typeDefinitionFactory
        ...
        addCustomTypes(UUID.fromString(zerotype), documentType, true);
    }
    
    private void addCustomTypes(UUID parent, TypeDefinition parentType, boolean recursion) throws Exception {
	    Page<SystemDTO> types = systemService.findAllByParentAndType(parent, "type", createPageRequest());
	    for (SystemDTO type : types) {
            switch (type.getSymbolicName()) {
                case FOLDER:
                    parentType = folderType;
                    break;
                case ABSTARCT_SYSTEM:
	    	...
	    	MutableTypeDefinition  childType = getTypeDefenitionFromType(type, parentType);
	        addTypeDefinition(childType);
	
	        if(recursion) addCustomTypes(type.getId(), childType, recursion);
	    }
    }
    ...
    private MutableTypeDefinition getTypeDefenitionFromType(SystemDTO type, TypeDefinition parentType) {
        LOGGER.debug("addCustomTypes(): type {} - {} ({})", type.getId(), type.getTitle(), type.getSymbolicName());
    	MutableTypeDefinition  childType = null;
    	
    	//if (parentType.getBaseTypeId().equals(folderType.getBaseTypeId())){
    		childType = typeDefinitionFactory
    				.createChildTypeDefinition(parentType, type.getSymbolicName());
    	//}else{
        ...
        while (nodes.hasNext()) {
          ...
          if (entry.getValue().get("type") != null && entry.getValue().get("type").asText().toLowerCase().equals("number")){
        	  proptype = PropertyType.DECIMAL; 
          }
          if (entry.getValue().get("type") != null && entry.getValue().get("type").asText().toLowerCase().equals("integer")){
        	  proptype = PropertyType.INTEGER; 
          }
          if (entry.getValue().get("type") != null && entry.getValue().get("type").asText().toLowerCase().equals("string")){
        	  if (entry.getValue().get("format") != null){
        		  String format = entry.getValue().get("format").asText().toLowerCase();
        		  if (format.equals("date-time")||format.equals("date")||format.equals("time")){
        			  proptype = PropertyType.DATETIME;
        		  }
        	  }
          }
     ...
     */
    private synchronized void addTypeDefinition(TypeDefinition type) {
        if (type == null) {
    ...
    private void checkModifiedTypes() throws Exception {
    	Map<String, SystemDTO> types = systemService.getModifiedTypes();
    	for (Entry<String, SystemDTO> type : types.entrySet()) {
    		LOGGER.debug("checkModifiedTypes {}",type.getKey());
    		SystemDTO typeDTO = type.getValue();
    		UUID parentId = typeDTO.getParent();
        	TypeDefinition parentType = documentType;
        	if (!Objects.equals(parentId.toString(), zerotype)){
        		String parName = systemService.findById(parentId).getSymbolicName();
        		LOGGER.debug("Parent type name - {}",parName);
        		parentType = typeDefinitions.get(parName);
        	}
        	MutableTypeDefinition  childType = getTypeDefenitionFromType(typeDTO, parentType);
        	addTypeDefinition(childType);
        	
	        addCustomTypes(typeDTO.getId(), childType, true);
    	}
    	systemService.emptyModifiedTypes();
    	
    }
    
    public TypeDefinition getTypeDefinition(CallContext context, String typeId) throws Exception {
    	LOGGER.debug("getTypeDefinition {}",typeId);
    	checkModifiedTypes();
    ...
    public TypeDefinitionList getTypeChildren(CallContext context, String typeId, Boolean includePropertyDefinitions,
    	...
    	LOGGER.debug("getTypeChildren {}",typeId);
    	checkModifiedTypes();
    ...
    public List<TypeDefinitionContainer> getTypeDescendants(CallContext context, String typeId, BigInteger depth,
    	...
    	LOGGER.debug("getTypeDescendants {}",typeId);
    	checkModifiedTypes();
