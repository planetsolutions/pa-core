...
abstract class BridgeRepository {
    ...
    static final Pattern IN_FOLDER_QUERY_PATTERN = Pattern
            .compile("(?i)select\\s+(\\S*).*\\s+from\\s+(\\S*).*\\s+where\\s+in_folder\\('(.*)'\\)");

	static final String STORAGE_DATE_FORMAT = "yyyy-MM-dd'T'HH:mm:ssX";
...
//    Map<String, Boolean> readWriteUserMap;
    ...
    static String getId(String docID) {
    ...
    private Properties compileProperties(CallContext context, File file, Set<String> orgfilter,
        ...
        return compileProperties(context, rootId, orgfilter, objectInfo, isDirectory, id, name, null,
                ...
                isFileNotEmpty, streamMimeType, streamFileName, streamLenght, null, null, id);
    ...
    private Properties compileProperties(CallContext context, DocumentDTO doc, DocumentDTO parentDoc, Set<String> orgfilter,
        ...
        String rootId = isDirectory ? (parentDoc != null ? getId(parentDoc.getId().toString()) : null):null;
        String id = getId(doc.getId().toString());
        ...
        String streamFileName = isDocHasContent ? doc.getFileName() : null;
        ...
        UUID seriesId = doc.getVersionSeries();
        if (seriesId==null){
        	seriesId=doc.getId();
        }
        return compileProperties(context, rootId, orgfilter, objectInfo, isDirectory, id, name, description,
                ...
                isDocHasContent, streamMimeType, streamFileName, streamLenght, customType, docData, seriesId.toString());
    ...
    private Properties compileProperties(CallContext context, String rootId, Set<String> orgfilter, ObjectInfoImpl objectInfo,
                                         ...
                                         String customType, JsonNode docData, String seriesId){
        ...
        try {
            ...
            for(Entry<String, PropertyDefinition<?>> prop: type.getPropertyDefinitions().entrySet()){
            	...
            	if (docData.get(propName)!=null){
            		...
            		if (propDef.getPropertyType().equals(PropertyType.DECIMAL))
            			addPropertyBigDecimal(result, typeId, filter, propName, BigDecimal.valueOf(docData.get(propName).asDouble()), type);
            		if (propDef.getPropertyType().equals(PropertyType.DATETIME)){
            			GregorianCalendar cal = new GregorianCalendar();
            			try{
	            			DateFormat df = new SimpleDateFormat(STORAGE_DATE_FORMAT);
	            			Date date = df.parse(docData.get(propName).textValue());
	            			cal.setTime(date);
            			}catch(Exception e){
            				LOGGER.error("Unparseable date: "+docData.get(propName).textValue());
            			}
            			addPropertyDateTime(result, typeId, filter, propName, cal, type);
            		}
            	}
            }
            }
            ...
            if (isDirectory) {
                ...
                // version series id
                addPropertyId(result, typeId, filter, PropertyIds.VERSION_SERIES_ID, seriesId, type);
                objectInfo.setId(seriesId);
...
//        }
    ...
    String getUserNameFromJwtToken(CallContext context){
    	...
    	final JwtTokenHelper.UserCredentials userCredentials = JwtTokenHelper.INSTANCE.getUserFromJWT(jwtToken);
    	if(userCredentials == null) {
    		return context.getUsername();
        }
        return userCredentials.getUserName();
    }
    String getJwtTokenFromCallContext(CallContext context){

        String token = null;
        final HttpServletRequest request = (HttpServletRequest) context.get(CallContext.HTTP_SERVLET_REQUEST);
    ...
    List<String> getUserRolesFromToken(final String token) {
        ...
        return userCredentials.getUserRoles();
    }
