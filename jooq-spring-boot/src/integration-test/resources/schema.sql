...
create table if not exists documents (
  ...
  sys_uuid uuid UNIQUE NOT NULL DEFAULT uuid_generate_v4(),
  sys_tenant text,
  sys_application text,
  sys_symbolic_name text,
  sys_source_id text,
  sys_acl jsonb,
  sys_status text,
  sys_parent_uuid uuid,
  sys_source_package text,
  sys_retention_policy text,
  ver_islast boolean,
  ver_series_uuid uuid,
  ver_parent_uuid uuid,
  ver_comment text,
  data jsonb
);

-- ALTER TABLE public.documents ADD CONSTRAINT uuid_unique UNIQUE(sys_uuid);

CREATE TABLE if not exists links (
	head_id		uuid REFERENCES documents (sys_uuid),
	tail_id		uuid REFERENCES documents (sys_uuid),
	link_type text,
	data jsonb,
...
(
 ...
 GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE public.documents TO doccloud_it;
 GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE public.links TO doccloud_it;
 GRANT SELECT, UPDATE, INSERT ON TABLE public.users TO doccloud_it;
 GRANT SELECT, UPDATE, INSERT ON TABLE public.user_roles TO doccloud_it;
 GRANT SELECT, UPDATE, INSERT ON TABLE public.groups TO doccloud_it;
 GRANT SELECT, UPDATE, INSERT ON TABLE public.roles TO doccloud_it;
